...
// Extract batch ID from either a raw ID or a full verify URL
const extractBatchId = (raw: string): string => {
  const trimmed = raw.trim();
  // Handle encoded URL: https://agrilink.in/verify/AGRL-...
...
const buildLookup = () => {
  ...
  // Buyer-side listings (includes extra metadata like farmerVerified, certifications)
  ...
  // Farmer store listings — covers newly added listings not yet in static data
  farmerListingsStore.getAll().forEach(l => {
    if (!lookup[l.batchId]) {
      lookup[l.batchId] = {
        status: 'authentic',
        crop: `${l.name} (${l.variety})`,
        ...
        harvestDate: l.harvestDate,
        batchId: l.batchId,
        certifications: ['AgriLink Verified'],
      };
    }
  });
...
function CameraScanner({ onResult, onClose, batchIds }: { onResult: (text: string) => void; onClose: () => void; batchIds: string[] }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      // Simulate scanning a QR that encodes the full verify URL (as generated by farmerListingsStore)
      const randomBatchId = batchIds[Math.floor(Math.random() * batchIds.length)] || 'AGRL-2025-TOM-001';
      onResult(`https://agrilink.in/verify/${randomBatchId}`);
    }, 3000);
...
export default function QualityVerification() {
  ...
  const verify = useCallback((raw: string) => {
    ...
    setTimeout(() => {
      setLoading(false);
      const found = batchLookup[id];
      if (found) { setBatchId(id); setResult(found); }
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeout(() => {
      ...
      const detected = batchIds[Math.floor(Math.random() * batchIds.length)];
      setBatchId(detected);
    ...
    <div className="max-w-2xl mx-auto animate-fade-in">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-2xl font-black text-gray-900">{t('qualityVerification.title')}</h1>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8 mb-6">
        ...
        <div className="flex gap-3 mb-5">
          <button onClick={() => setShowCamera(true)}
            ...
            <Camera className="w-4 h-4" /> {t('qualityVerification.openCamera')}
          </button>
          <button onClick={() => fileRef.current?.click()}
            ...
            <Upload className="w-4 h-4" /> {t('qualityVerification.uploadQr')}
          </button>
        ...
        <div className="space-y-3">
          ...
          <button onClick={handleVerify} disabled={!batchId.trim() || loading}
            ...
            {loading ? (
              <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> {t('qualityVerification.verifying')}</>
            ) : (
              <><ShieldCheck className="w-4 h-4" /> {t('qualityVerification.verifyBatch')}</>
            )}
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {notFound && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <XCircle className="w-12 h-12 text-red-400 mx-auto mb-3" />
            <p className="font-bold text-red-700">{t('qualityVerification.batchNotFound')}</p>
            <p className="text-sm text-red-500 mt-1">{t('qualityVerification.batchNotFoundDesc', { id: batchId })}</p>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {result && cfg && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div className={`${cfg.bg} p-6 space-y-3`}>
              {[
                { icon: Leaf, label: t('qualityVerification.crop'), value: result.crop },
                { icon: Award, label: t('qualityVerification.grade'), value: result.grade },
                { icon: User, label: t('qualityVerification.farmer'), value: result.farmer },
                { icon: MapPin, label: t('qualityVerification.location'), value: result.location },
                { icon: Calendar, label: t('qualityVerification.harvestDate'), value: result.harvestDate },
